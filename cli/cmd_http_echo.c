...
{
    fprintf(out, "ffnet http-echo — HTTP/1.1 echo server\n\n");
    fprintf(out, "usage:\n");
    fprintf(out, "  ffnet http-echo --listen <addr>:<port> [--workers N]\n\n");
    fprintf(out,
        "Echoes the request body back as the response body. Exercises\n"
...
int cmd_http_echo(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--listen") == 0 && i + 1 < argc) {
            ...
            fprintf(stderr, "http-echo: unknown arg: %s\n\n", argv[i]);
    ...
    if (parse_listen(listen_spec, host, sizeof host, &port) != FFE_OK) {
        fprintf(stderr, "http-echo: bad --listen spec: %s\n", listen_spec);
        return 2;
    }
